...
def _fmt_mla(paper) -> str:
    ...
    if not authors:
        ...
        if len(first) == 2:
            author_str = f"{first[-1]}, {first[0]}, et al."
        else:
...
class QAView:
    ...
    def build(self) -> ft.Control:
        ...
        pdf_panel = self._build_pdf_panel()
        ...
        return ft.Column(
            [
                ...
                ft.Text(
                    "根據論文庫回答問題，有上傳 PDF 全文的論文會提供更精準的章節級引用",
    ...
    def _build_pdf_panel(self) -> ft.Control:
        self._pdf_status_text = ft.Text("", size=12)
        self._source_status = ft.Text(
            "（目前：使用全部論文）", size=11, color=ft.colors.GREY_600, italic=True
        )

        # ── 論文清單（兼做來源選擇）────────────────────────────────
        self._paper_list_col = ft.Column(spacing=4, scroll=ft.ScrollMode.AUTO)
        self._rebuild_paper_list()
        self._on_source_changed()  # 套用預設勾選狀態
        ...
        self._pdf_panel_content = ft.Column([
            # 標題列
            ft.Row([
                ft.Icon("picture_as_pdf", color=ft.colors.RED_700),
                ft.Text("全文管理與問答來源", size=14, weight=ft.FontWeight.W_600),
                ft.Container(expand=True),
                toggle_btn,
            ]),

            # 論文清單（勾選 = 參與問答 / 每列可上傳或刪除全文）
            ft.Container(
                content=ft.Column([
                    ft.Row([
                        ft.Text("勾選要參與問答的論文", size=12,
            ...
            ft.Container(
                content=ft.Column([
                    ft.Row([
                        ft.Text("匯入 PDF 全文", size=12,
                                weight=ft.FontWeight.W_600, color="#1B5E20"),
                        ft.Container(expand=True),
                        add_pdf_btn,
                        import_all_btn,
                    ], spacing=8),
                    ft.Text(
                        "選擇一或多個 PDF，為每個檔案指定對應論文後按「一鍵匯入全部」",
            ...
            ft.Container(
                content=ft.Column([
                    ft.Row([
                        ft.Icon("auto_awesome", color="#6A1B9A", size=16),
                        ft.Text("從 PDF 自動建立新論文", size=12,
                                weight=ft.FontWeight.W_600, color="#6A1B9A"),
                        ft.Container(expand=True),
                        new_from_pdf_btn,
                    ], spacing=8),
                    ft.Text(
                        "上傳 PDF 後，AI 自動萃取標題、作者、年份、摘要、標籤，確認後新增論文並完成全文向量化",
                        size=11, color=ft.colors.GREY_600,
                    ),
                ], spacing=4),
                bgcolor="#F3E5F5",
    ...
    def _rebuild_paper_list(self):
        ...
        for p in papers:
            ...
            title = p.title[:58] + "…" if len(p.title) > 58 else p.title
            year = f" ({p.year})" if p.year else ""

            badge = ft.Container(
                content=ft.Text("全文" if has_ft else "摘要", size=9, color=ft.colors.WHITE),
            ...
            row = ft.Row([
                cb, badge,
                ft.Text(title + year, size=11, expand=True,
                        overflow=ft.TextOverflow.ELLIPSIS, max_lines=1),
                chunks_label,
                action_btn,
            ], spacing=4, vertical_alignment=ft.CrossAxisAlignment.CENTER)

            self._paper_list_col.controls.append(row)

    def _on_source_changed(self):
        """勾選框變動時立即更新 _source_paper_ids"""
        selected = {pid for pid, cb in self._source_checkboxes.items() if cb.value}
        all_ids = set(self._source_checkboxes.keys())
        if not selected:
            self._source_paper_ids = set()   # 空集合 = 問答時會提示無來源
            self._source_status.value = "⚠️ 未選任何論文（問答將無法回答）"
            ...
            self._source_status.value = f"✓ 已選 {len(selected)} / {len(all_ids)} 篇參與問答"
            self._source_status.color = ft.colors.GREEN_700
        try:
            if self._source_status and self._source_status.page:
    ...
    def _toggle_pdf_panel(self, e):
        self._pdf_panel_visible = not self._pdf_panel_visible
        controls = self._pdf_panel_content.controls
        for ctrl in controls[1:]:
            ctrl.visible = self._pdf_panel_visible
        self._toggle_btn.text = "▼ 收起" if self._pdf_panel_visible else "▶ 展開"
        self.page.update()
    ...
    def _make_paper_options(self) -> list[ft.dropdown.Option]:
        ...
        return [
            ft.dropdown.Option(
                str(p.id),
                f"[{p.id}] {p.title[:55]}{'…' if len(p.title) > 55 else ''} ({p.year or '?'})",
            )
            for p in papers
        ]

    def _pick_multiple_pdfs(self, e):
        """讓使用者一次選多個 PDF，加入上傳佇列"""
    ...
    def _on_import_all(self, e):
        ...
        self._pdf_status_text.value = f"⏳ 正在匯入 {len(pending)} 個 PDF..."
        self._pdf_status_text.color = ft.colors.ORANGE_700
        self.page.update()
        ...
        def run():
            ...
            for en in pending:
                ...
                def _prog(msg, name=en["name"]):
                    ...
                    self._pdf_status_text.color = ft.colors.ORANGE_700
                    self.page.update()
                result = ingestor.ingest(en["path"], pid, replace_existing=True,
                                        progress_callback=_prog)
                if result.success:
                    ok += 1
                    msgs.append(f"✅ {en['name']}：{result.total_chunks} chunks")
                else:
                    fail += 1
                    msgs.append(f"❌ {en['name']}：{result.error}")
            ...
            self._pdf_status_text.color = ft.colors.GREEN_700 if fail == 0 else ft.colors.ORANGE_700
            self._rebuild_paper_list()
            self.page.update()
    ...
    def _pick_single_pdf(self, paper_id: int):
        """從論文列表的上傳按鈕點進來，直接上傳該篇"""
        def on_result(result):
            if not result.files or not result.files[0].path:
                return
            f = result.files[0]
            self._pdf_status_text.value = f"⏳ 正在解析 {f.name}..."
            self._pdf_status_text.color = ft.colors.ORANGE_700
            self.page.update()
            ...
            def run():
                ...
                if res.success:
                    self._pdf_status_text.value = (
                        f"✅ {f.name} 匯入成功：{res.total_chunks} chunks，"
                        f"{res.table_chunks} 表格，{res.total_pages} 頁"
                    )
    ...
    def _pick_pdf_for_new_paper(self, e):
        """讓使用者選一個 PDF，自動萃取 metadata 後彈出確認對話框"""
        def on_result(result):
            if not result.files or not result.files[0].path:
                return
            f = result.files[0]
            self._pdf_status_text.value = f"⏳ AI 分析中：{f.name}..."
    ...
    async def _show_meta_dialog(self, pdf_path: str, filename: str, meta: ExtractedMeta):
        """彈出 Metadata 確認對話框，讓使用者可以編輯後確認匯入"""
        title_f = ft.TextField(
            label="標題 *", value=meta.title, multiline=True, min_lines=1, max_lines=3,
        )
        authors_f = ft.TextField(
            label="作者（逗號分隔）",
            value=", ".join(meta.authors),
            multiline=False,
        )
        year_f = ft.TextField(
            label="年份", value=str(meta.year) if meta.year else "", width=100,
        )
        venue_f = ft.TextField(
            label="期刊/會議", value=meta.venue, expand=True,
        )
        doi_f = ft.TextField(
            label="DOI（選填）", value=meta.doi, expand=True,
        )
        abstract_f = ft.TextField(
            label="摘要", value=meta.abstract, multiline=True,
        ...
        def _collect_meta() -> ExtractedMeta:
            authors_raw = authors_f.value or ""
            authors = [a.strip() for a in authors_raw.split(",") if a.strip()]
            year_raw = year_f.value.strip() if year_f.value else ""
            try:
                year = int(year_raw) if year_raw else None
            except ValueError:
                year = None
            tags_raw = tags_f.value or ""
        ...
        dlg = ft.AlertDialog(
            modal=True,
            title=ft.Text(f"確認論文資訊：{filename}", size=15, weight=ft.FontWeight.W_600),
            ...
            actions=[
                ...
                ft.ElevatedButton(
                    ...
                    icon="save",
                    style=ft.ButtonStyle(bgcolor="#6A1B9A", color=ft.colors.WHITE),
                    on_click=lambda e: self._do_import(dlg, pdf_path, _collect_meta, status_text),
                ),
            ],
    ...
    def _do_import(self, dlg: ft.AlertDialog, pdf_path: str, collect_meta_fn, status_text: ft.Text):
        """點「確認匯入」後在背景執行匯入"""
        ...
        def run():
            ...
            importer = self._get_importer()
            result = importer.import_from_pdf(pdf_path, confirmed_meta, progress_callback=_prog)

            if result.success:
                dlg.open = False
                self._pdf_status_text.value = (
                    f"✅ 已建立新論文 [ID={result.paper_id}]：{confirmed_meta.title[:50]}"
                    + (f"，{result.total_chunks} chunks" if result.total_chunks else "")
                    + (f"（注意：{result.error}）" if result.error else "")
                )
                self._pdf_status_text.color = ft.colors.GREEN_700
                self._rebuild_paper_list()
                self._on_source_changed()
            else:
                status_text.value = f"❌ 匯入失敗：{result.error}"
                status_text.color = ft.colors.RED_700
            self.page.update()
    ...
    def _build_compare_panel(self) -> ft.Container:
        """建構跨論文比較面板"""
        self._compare_col = ft.Column(spacing=4, scroll=ft.ScrollMode.AUTO)
        self._compare_status = ft.Text("", size=12)
        self._compare_q = ft.TextField(
            hint_text="比較問題（選填，如：這幾篇論文的方法有什麼根本差異？）",
            multiline=False, expand=True, height=38,
            border_radius=8,
        ...
        return ft.Container(
            content=ft.Column([
                ft.Row([
                    ft.Icon("compare_arrows", color="#0277BD", size=16),
                    ft.Text("跨論文比較分析", size=12,
                            weight=ft.FontWeight.W_600, color="#0277BD"),
                    ft.Container(expand=True),
                    ft.TextButton("全選", on_click=lambda e: self._toggle_all_compare(True)),
                    ft.TextButton("全不選", on_click=lambda e: self._toggle_all_compare(False)),
                    compare_btn,
                ], spacing=6),
                ft.Text(
                    "勾選 2-6 篇論文，AI 比較方法異同、數據差異、研究缺口（會顯示在對話框中）",
                    size=11, color=ft.colors.GREY_600,
                ),
                self._compare_q,
                ft.Container(
    ...
    def _on_compare(self, e):
        ...
        if len(selected_ids) < 2:
            self._compare_status.value = "⚠️ 請至少勾選 2 篇論文"
            self._compare_status.color = ft.colors.ORANGE_700
            self.page.update()
            return
        if len(selected_ids) > 6:
            self._compare_status.value = "⚠️ 最多選 6 篇"
            self._compare_status.color = ft.colors.ORANGE_700
            self.page.update()
            return

        question = (self._compare_q.value or "").strip()
        self._compare_status.value = f"⏳ 分析 {len(selected_ids)} 篇論文中..."
        ...
        def run():
            ...
            self._compare_status.value = f"✅ 比較完成，結果已顯示在對話框"
            self._compare_status.color = ft.colors.GREEN_700

            # 把結果放進對話框
            titles = "、".join(p.title[:20] + "…" for p in result.papers)
            qa_result = QAResult(
                answer=result.raw_answer,
                source_chunks=[],
                query=question or f"比較 {len(result.papers)} 篇論文",
            )
            # 用特殊樣式的卡片顯示
    ...
    def _append_compare_result(self, result, titles: str):
        ...
        inner_controls = [
            ft.Row([
                ft.Icon("compare_arrows", size=16, color="#0277BD"),
            ...
            inner_controls.append(
                ft.Container(
                    content=ft.Column([
                        ...
                        ft.Text(content[:600] + ("…" if len(content) > 600 else ""),
                                size=12, selectable=True, color=ft.colors.GREY_800),
                    ], spacing=4),
                    bgcolor="#FAFAFA",
        ...
        bubble = ft.Container(
            content=ft.Column(inner_controls, spacing=8),
            bgcolor="#E1F5FE",
            border=ft.border.all(1, "#81D4FA"),
            border_radius=8,
            padding=14,
    ...
    def _on_send(self, e):
        ...
        if isinstance(self._source_paper_ids, set) and len(self._source_paper_ids) == 0:
            self._append_error("請先在上方勾選至少一篇參與問答的論文。")
            return
        ...
        # 多輪記憶：若問題是追問，注入上一次引用的論文作為優先候選
        context_paper_ids: Optional[set[int]] = None
        if self._last_result and _is_followup(question):
            prev_ids = {sc.paper.id for sc in self._last_result.source_chunks if sc.paper.id}
            if prev_ids:
            ...
            content=ft.Column(
                [
                    ...
                    ft.Text(
                        ...
                        "• 有哪些論文的實驗結果超過 90% accuracy？（需上傳全文）",
                        size=13, color=ft.colors.GREY_700,
                    ),
                ],
                spacing=8,
        ...
        bubble = ft.Container(
            content=ft.Column(
                [
                    ft.Row(
                        [
                            ft.Icon("person", size=16, color=ft.colors.BLUE_700),
                            ft.Text("你", size=12, weight=ft.FontWeight.W_600,
        ...
        bubble = ft.Container(
            content=ft.Column(
                [
                    ft.Row(
                        [
                            ft.Icon("smart_toy", size=16, color=ft.colors.PURPLE_700),
                            ft.Text("AI 助理", size=12, weight=ft.FontWeight.W_600,
                                    color=ft.colors.PURPLE_700),
                        ],
                        spacing=4,
                    ),
                    ft.Text(result.answer, size=14, selectable=True),
                    *([ft.Divider(height=6, color=COLOR_BORDER)] + source_controls
                      if source_controls else []),
                ],
                spacing=6,
            ),
            bgcolor=COLOR_ASST_BG,
            border_radius=8,
            padding=12,
        )
        self._chat_column.controls.append(bubble)
    ...
    def _build_source_controls(self, source_chunks: list[SourceChunk]) -> list[ft.Control]:
        ...
        # 去重：同論文只顯示一次，但列出所有用到的 section
        paper_sections: dict[int, list[SourceChunk]] = {}
        for i, sc in enumerate(source_chunks, 1):
            pid = sc.paper.id
            if pid not in paper_sections:
                paper_sections[pid] = []
            paper_sections[pid].append((i, sc))

        citation_num = 1
        for pid, chunks in paper_sections.items():
            ...
            def _copy_apa(e, txt=apa_text):
                self.page.set_clipboard(txt)

            def _copy_mla(e, txt=mla_text):
                self.page.set_clipboard(txt)
                ...
                ft.Container(
                    content=ft.Row(
                        [
                            ft.Container(
                                content=ft.Text(
                                    str(citation_num), size=11, color=ft.colors.WHITE,
                            ...
                            ft.Text(
                                f"{title}{year_str}",
                                size=12, color=ft.colors.GREY_800, expand=True,
                                overflow=ft.TextOverflow.ELLIPSIS, max_lines=1,
                            ),
                            ft.Row(section_badges, spacing=4),
                            ft.Tooltip(
                                message=f"複製 APA：\n{apa_text}",
                                content=ft.TextButton(
                                    "APA", style=ft.ButtonStyle(
    ...
    def _append_error(self, msg: str):
        ctrl = ft.Container(
            content=ft.Text(msg, size=13, color=ft.colors.RED_700),
            bgcolor="#FFEBEE", border_radius=8, padding=12,
        )
        self._chat_column.controls.append(ctrl)
        self._chat_column.update()
